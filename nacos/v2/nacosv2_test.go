...
func TestRegistryAndDeregister(t *testing.T) {
	register := NewNacosV2Registry(namingClient)
	infos := []registry.Info{
		{
			ServiceName: "service-name",
			Addr:        utils.NewNetAddr("tcp", "127.0.0.1:8849"),
			Weight:      10,
			Tags:        nil,
		},
		{
		...
		{
			ServiceName: "service-name",
			Addr:        utils.NewNetAddr("tcp", "127.0.0.1:&(#"),
			Weight:      10,
			Tags:        nil,
		},
		{
		...
		{
			ServiceName: "service-name",
			Addr:        utils.NewNetAddr("tcp", "127.0.0.1:884959829651"),
			Weight:      10,
			Tags:        nil,
		},
	}
...
func TestNewDefaultResolver(t *testing.T) {
	r, err := NewDefaultNacosV2Resolver()
	assert.NotNil(t, r)
	assert.Nil(t, err)
}
...
func TestMultipleInstances(t *testing.T) {
	...
	time.Sleep(time.Second)
	got := NewNacosV2Registry(namingClient, WithRegistryCluster(clusterName), WithRegistryGroup(groupName))
...
func TestResolverResolve(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			n := NewNacosV2Resolver(tt.fields.cli)
	...
	err := NewNacosV2Registry(namingClient).Deregister(&registry.Info{
		ServiceName: "demo.hertz-contrib.local",
		Addr:        utils.NewNetAddr("tcp", "127.0.0.1:8080"),
		Weight:      10,
	})
...
func TestDefaultNacosRegistry(t *testing.T) {
	register, err := NewDefaultNacosV2Registry()
	assert.Nil(t, err)
	info := &registry.Info{
		ServiceName: "service-name",
