...
// NewDefaultNacosV2Resolver create a default service resolver using nacos.
func NewDefaultNacosV2Resolver(opts ...ResolverOption) (discovery.Resolver, error) {
	client, err := common.NewDefaultNacosV2Config()
	if err != nil {
		return nil, err
	}
	return NewNacosV2Resolver(client, opts...), nil
}

// NewNacosV2Resolver create a service resolver using nacos.
func NewNacosV2Resolver(cli naming_client.INamingClient, opts ...ResolverOption) discovery.Resolver {
	opt := resolverOptions{
		cluster: "DEFAULT",
		group:   "DEFAULT_GROUP",
	}
